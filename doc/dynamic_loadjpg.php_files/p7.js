// JavaScript Document
// This is done on click, passing one parameter: this
function P7_TMenu(b,og) { //v2.5 by Project Seven Development(PVII)
  var i,s,c,k,j,tN,hh;
//  alert('b='+b+' og='+og);
  // b = javascript.void() and og=1 when page is first opened (called from where?)
  // when main nav button clicked after that, b=javascript.void() and og=undefined
...
// This is called for any page that opens with the js in the head of that page
// writes a bunch of div tags in the head section (don't show in view source, only if
// page is saved)
...
// called in body onload for all pages
function P7_TMopen() {
  var i,x,d,hr,ha,ef,a,ag;
  if (document.getElementById) {
  	// div id=p7TMnav is nav section
    ...
    if (d) {
      ...
      if (ha && ha.length) {
        for (i=0; i < ha.length; i++) {
          if (ha[i].href) {
		  	// if the current url is the one this link points to, assign a class to the link
			// that will cause it to stand out (bold here)
...
function P7_TMclass(){ //v2.5 by Project Seven Development(PVII)
 ...
 if(document.getElementById){
	...
	if(d){
		...
		if(tN&&tN.length){
			for(i=0;i<tN.length;i++){
				// find out if this was the item clicked?
