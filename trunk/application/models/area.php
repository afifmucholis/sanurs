...
class Area extends CI_Model {
     ...
     * @return type
     */
    function addArea($options = array()) {
        $fieldArray = array($this->name, $this->latitude, $this->longitude);
    ...
    /**
     * Method updateArea : update record pada tabel Area yang memenuhi id tertentu.
     * 
     * option: values
     * --------------
     ...
     * @return bool/int  
     */
    function updateArea($options = array()) {
        // required (id harus ada) :
        if (!$this->_required(array($this->id), $options))
            return false;

        //Set dari field :
        $fieldArray = array($this->count);
        foreach ($fieldArray as $field) {
            if (isset($options[$field])) {
                $this->db->set($field, $options[$field]);
            }
    ...
    /**
     * Method getAreas, mengembalikan array of area.
     * 
     * option : values
     * ---------------
     ...
     * @return array result() 
     */
    function getAreas($options = array()) {
        //nilai default :
    ...
    /**
     * Method delete area berdasarkan id.
     * 
     * @param array $options
     * @return type 
     */
    function deleteArea($options = array()) {
        //required value :
        if (!$this->_required(array($this->id), $options)) {
            return false;
        }
